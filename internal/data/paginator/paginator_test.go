...
func TestPaginator(t *testing.T) {
	t.Run("should return a list of items via pagination", func(t *testing.T) {
		...
		opts := Opts{ListOptions: &github.ListOptions{Page: 1, PerPage: 10}}

		resp, err := Paginator[*github.Repository](context.Background(), lFunc.List, pFunc.Process, rFunc.RateLimit, &opts)
		assert.NoError(t, err)
	...
	t.Run("should return when ratelimter returns a false response", func(t *testing.T) {
		...
		lFunc := &listFunc{client: client}
		opts := Opts{ListOptions: &github.ListOptions{Page: 1, PerPage: want}}
	...
	t.Run("should use default opts if none provided", func(t *testing.T) {
		...
		resp, err := Paginator[*github.Repository](context.Background(), lFunc.List, pFunc.Process, rFunc.RateLimit, nil)
		assert.NoError(t, err)
		assert.Len(t, resp, 378)
	})
	t.Run("should use 100 per page if per page is 0 (resource exhaustion)", func(t *testing.T) {
		...
		rFunc := &rateLimitFunc{}
		lFunc := &listFunc{client: client}
		opts := Opts{ListOptions: &github.ListOptions{Page: 1, PerPage: 0}}
	...
	t.Run("should return any error encountered by the rate limit function", func(t *testing.T) {
		...
		_, err = Paginator[*github.Repository](context.Background(), lFunc.List, pFunc.Process, rFunc.RateLimit, nil)
		assert.Error(t, err)
	})
